...
export class CompletionsResponseBuilder {
   ...
   */
  public nextToolCalls(funcCalled: CompletionFunctionCallsData[] | undefined | null): CompletionContentData[] {
    if (funcCalled) {
  ...
  private processToolCall(func: CompletionFunctionCallsData): void {
    ...
    if (id && fnData?.name) {
      ...
      if (!existingTool) {
        ...
        existingTool.tool_call.arguments = ((existingTool.tool_call.arguments as string) || '') + args;
      }
