...
import { addClientInfoParams } from '../..';
...
export type FireworksCodeCompletionParams = Record<string, any>;
...
export interface CompletionLogger {
  startCompletion(
    ...
    | {
        ...
        onFetch: (httpClientLabel: string, body: SerializedCompletionParameters | FireworksCodeCompletionParams) => void;
      };
}
...
/**
 * Access the chat based LLM APIs via a Cognix server instance.
 *
 * 🚨 SECURITY: It is the caller's responsibility to ensure context from
 * all cognix ignored files are removed before sending requests to the server.
 */
export abstract class CognixCompletionsClient {
  private errorEncountered = false;

  protected readonly isTemperatureZero = process.env.COGNIX_TEMPERATURE_ZERO === 'true';

  constructor(protected logger?: CompletionLogger) {}

  protected async completionsEndpoint(): Promise<string> {
    return new URL('/.api/completions/stream', (await currentResolvedConfig()).auth.serverEndpoint).href;
  }
  ...
  protected async prepareRequest(
    ...
    if (interactionId) {
      headerParams['X-Cognix-Interaction-ID'] = interactionId;
    }
  ...
  public async *stream(
    ...
    // Custom chat clients for Non-Cognix-supported providers.
    const isNonCognixProvider = await useCustomChatClient({
