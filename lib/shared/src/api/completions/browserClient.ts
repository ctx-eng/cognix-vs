...
import { addClientInfoParams, addCognixClientIdentificationHeaders } from '../client-name-version';
import { addAuthHeaders } from '../utils';

import { verifyResponseCode } from '../graphql/client';
import { CompletionsResponseBuilder } from './CompletionsResponseBuilder';
import { type CompletionRequestParameters, CognixCompletionsClient } from './client';
...
export class CognixBrowserCompletionsClient extends CognixCompletionsClient {
  protected async _streamWithCallbacks(
    ...
    addCognixClientIdentificationHeaders(headersInstance);
    ...
    if (trace) {
      headersInstance.set('X-Cognix-Should-Trace', 'true');
    }
    ...
    fetchEventSource(url.toString(), {
      method: 'POST',
      headers: Object.fromEntries((headersInstance as any).entries()),
      ...
      onerror(error: any) {
        cb.onError(new Error(error.message));
        abort.abort();
        console.error(error);
        // throw the error for not retrying
        throw error;
...
if (isRunningInWebWorker) {
  ...
  (self as any).document = {
    ...
    // available in a web worker, without this cognix demo fails in dev mode.
    querySelectorAll: () => [],
  };
  (self as any).window = {
  ...
  // HACK: @openctx/vscode-lib uses global object to share vscode API, it breaks cognix web since
