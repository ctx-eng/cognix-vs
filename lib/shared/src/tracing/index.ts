import opentelemetry, { type Context, ROOT_CONTEXT, SpanStatusCode, context, propagation, type Span } from '@opentelemetry/api';
import type { BrowserOrNodeResponse } from '../api/graphql/client';

const INSTRUMENTATION_SCOPE_NAME = 'cognix';
  ...
  return tracer.startActiveSpan(name, (span): R => {
    ...
    try {
      ...
      if (typeof response === 'object' && response !== null && 'then' in response) {
        // @ts-expect-error Response seems to be a Thenable
